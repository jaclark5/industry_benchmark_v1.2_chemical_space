...
def main(argv=[__name__]):
    ...
    logger.info("Core fragment = %s" % oechem.OEMolToSmiles(bestcore))
...
def GetCoreFragment(
    ...
    logger.info("Number of molecules = %d" % len(mols))

    refmol = GetReferenceMolecule(mols)
    logger.info(f"Reference molecule: {oechem.OECreateSmiString(refmol)}")
    frags = GetFragments(refmol, minbonds, maxbonds)
    if len(frags) == 0:
        oechem.OEThrow.Error(
            "No fragment is enumertated with bonds %d-%d!" % (minbonds, maxbonds)
        )

    logger.info("Number of fragments = %d" % len(frags))

    commonfrags = GetCommonFragments(mols, frags, atomexpr, bondexpr)
    if len(commonfrags) == 0:
        logger.warning("No common fragment is found!")
        return None

    logger.info("Number of common fragments = %d" % len(commonfrags))
